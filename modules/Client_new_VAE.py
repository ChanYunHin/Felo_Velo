...
from modules.ResnetModel import SmallResnet1, SmallResnet2, SmallResnet3, Resnet50, SmallResnet50
...
class Client:
    ...
    def train_step(self, x, y,
        ...
        if distillation:
            if args.softmax_flag or args.KLD_flag:
                ...
                distill_loss = self.distill_loss_func_logits(log_softmax_predictions, 
                                                             softmax_logit_vectors)
            else:
            ...
            # features_classifier_loss = self.loss_func(features_predictions, y)
            feature_extractor_loss = self.distill_loss_func_features(encoded_features,
                                                                     server_features)
            # features_loss = features_classifier_loss + feature_extractor_loss
            features_loss = feature_extractor_loss
        ...
        if not no_record:
            ...
            if distillation:
                self.mean_loss.append(pred_loss.item() - alpha * features_loss.item())
                # self.feature_loss.append(alpha * features_loss.item())
            else:
    ...
    # def train_step_second(self, y, feature_predictions,
    ...
    #     features_loss = self.loss_func(feature_predictions, server_generated_labels)
        
    #     if args.softmax_flag or args.KLD_flag:
    #         softmax_server_feature_logits = F.softmax(server_feature_logits)
    #         log_softmax_features_predictions = F.log_softmax(feature_predictions)
    #         feature_distill_loss = self.distill_loss_func_logits(log_softmax_features_predictions, 
    #                                                              softmax_server_feature_logits)
    #     else:
    #         feature_distill_loss = self.distill_loss_func_logits(feature_predictions,
    #                                                              server_feature_logits)
        
    #     features_loss = features_loss + args.beta * feature_distill_loss
    #     pred_loss = pred_loss + alpha * (distill_loss + features_loss)
    ...
    #     gradients = [param.grad.detach() for param in self.model.parameters()]
    #     self.optimizer.zero_grad()
    #     # self.scheduler.step(pred_loss.item() - features_loss.item())

    #     self.distill_loss.append(distill_loss.item())
    #     self.feature_loss.append(features_loss.item())
    #     self.accuracy.append((y.eq(predictions.max(dim=1).indices).sum() / y.shape[0]).item())
    #     self.mean_loss.append(pred_loss.item() - cita * features_loss.item())
        
    #     training_result = [gradients, 
    #                        predictions.detach(), 
    #                        encoded_features.detach(),
    #                        feature_predictions.detach(),
    #                        features_loss.item()]
    #     return training_result
    ...
    #     server_features_vectors = self.match_labels_to_vectors(server_features, y)
    ...
    def training(self, 
        ...
        server_logit_vectors, server_feature_vectors = None, None
        client_y_logits = {}
        count_for_labels = {}
        self.mean_loss = []
        self.accuracy = []
        ...
        for i in range(self.clients_training_epoch):
            ...
            if distillation:
                server_logit_vectors = self.match_labels_to_vectors(server_y_logits, y)
                server_feature_vectors = self.match_labels_to_vectors(server_features, y)
            ...
            train_result = self.train_step(x,
                                        ...
                                        logit_vectors=server_logit_vectors,
                                        distillation=distillation,
                                        server_features=server_feature_vectors)
