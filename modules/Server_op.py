...
from modules.Client_op import Client
...
class Server:
    def __init__(self, parameters):
        ...
        self.clients_dict = self.build_clients_and_server(self.clients_number, 
                                                          self.distorted_data,
                                                          self.dataset_server)
        self.server_model = 0
    ...
    def get_server_client_features(self, res):
        ...
        for key, val in res["server_client_features"].items():
            server_features_dict[key] = torch.mean(torch.stack(val, dim=0), dim=0)
        return server_features_dict
    ...
    def asynchronous_training(self):
        ...
        alpha = args.alpha
        avg_server_y_logits, avg_server_client_features = 0, 0
        server_feature_logits = 0
        clients_y_logits = {}
        res = {}
        ...
        for ep in tqdm(range(max_training_epoches)):
            ...
            #     else:
            #         if ep >= int(max_training_epoches * 0.1) and ep % int(max_training_epoches * 0.01) == 0:
            #             distillation_flag = args.y_distillation_flag
            #             #training
            #             features_dataloader = self.get_server_client_features(res)
            #             self.server_extractor = CVAE.server_extractor_training(self.server_extractor,
            ...
            if distillation_flag:
                ...
                avg_server_client_features = self.get_server_client_features(res)
            ...
            for idx, cid in enumerate(completed_clients):
                ...
                training_result = train_model.training(distillation=distillation_flag,
                                                       random_seed=rand_seed,
                                                       latent_size=args.latent_size,
                                                       server_features=avg_server_client_features,
            ...
            if ep % eval_interval == 0:
                ...
                for idx in range(self.clients_number):
                    ...
                    torch.save(tmp_model.model.state_dict(), 'state_dict/Felo_{}_{}'.format(idx, self.dataset))
                ...
                for idx in range(len(group_model_acc)):
                    compared_summary_writer.add_scalars("group_accuracy", {'Felo_group{}'.format(idx): group_model_acc[idx]}, int(ep/eval_interval))
