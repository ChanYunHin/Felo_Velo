...
class ResNet18Enc(nn.Module):
    def __init__(self, z_dim=32, label_embedding_size=512,
                 num_Blocks=[2,2,2,2], nc=3):
        # super(ResNet18Enc, self).__init__(BasicBlock, [2, 2, 2, 2])
        # self.z_dim = z_dim
        # # self.ResNet18 = models.resnet18(pretrained=False)
        # self.num_feature = self.fc.in_features
        # self.fc = nn.Linear(self.num_feature + label_embedding_size, 2 * self.z_dim)
        ...
        self.fc = nn.Linear(512 + label_embedding_size, 2 * z_dim)
    ...
    def forward(self, x, label_embeddings):
        ...
        # x = torch.flatten(x, 1)
        x = x.view(x.size(0), -1)
        concat_vec = torch.cat([x, label_embeddings], dim=1)
        x = self.fc(concat_vec)
...
class ResNet18Dec(nn.Module):
    ...
    def __init__(self, num_Blocks=[2, 2, 2, 2], z_dim=32, nc=3,
        ...
        self.linear = nn.Linear(z_dim + label_embedding_size, 512)
...
class ServerModel(nn.Module):
    def __init__(self, num_classes):
        ...
        label_embedding_size = 512
        classifier_features = args.VAE_size
        self.VAE_latent_size = args.VAE_size
        self.label_embedding = nn.Embedding(num_classes, label_embedding_size)
        
        self.encoder = ResNet18Enc(z_dim=self.VAE_latent_size, 
                                   label_embedding_size=label_embedding_size)
        self.decoder = ResNet18Dec(z_dim=self.VAE_latent_size, 
                                   label_embedding_size=label_embedding_size)
    ...
    def encode(self, x, labels):
        ...
        concat_vec = self.encoder(x, label_embeddings)
        ...
        return mean, var, label_embeddings
